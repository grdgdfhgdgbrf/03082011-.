...
class DataManager:
    ...
    def create_referral(self, user_id: int, referrer_code: str) -> Tuple[bool, str]:
        try:
            ...
            if player.referrer_id:
                return False, "❌ У вас уже есть реферер!"
    ...
    def init_boosts(self):
        self.available_boosts = {
            ...
            "boost_3": Boost(
                ...
                description="Увеличивает стоимость продажи минералов на 50%",
                boost_type="gold_multiplier",
                value=1.5,
                duration=3600,
                price_gold=8000,
                price_premium=15
    ...
    def buy_boost(self, user_id: int, boost_id: str, pay_with_premium: bool = False) -> Tuple[bool, str]:
        try:
            ...
            boost = self.available_boosts.get(boost_id)
            if not boost:
                return False, "❌ Буст не найден"
            
            if pay_with_premium:
                if player.premium_coin_balance < boost.price_premium:
                    return False, f"❌ Недостаточно Premium Coin! Нужно: {boost.price_premium}"
                player.premium_coin_balance -= boost.price_premium
            else:
                if player.gold_balance < boost.price_gold:
            ...
            boost_copy = Boost(
                ...
                price_gold=boost.price_gold,
                price_premium=boost.price_premium,
                expires_at=datetime.now() + timedelta(seconds=boost.duration)
            )
            
            self.active_boosts[user_id].append(boost_copy)
            asyncio.create_task(self.batch_save())
            
            return True, f"✅ Буст '{boost.name}' активирован на {boost.duration // 60} минут!"
    ...
    def get_active_boosts(self, user_id: int) -> List[Boost]:
        ...
        for boost in self.active_boosts.get(user_id, []):
            if boost.expires_at and now < boost.expires_at:
                active.append(boost)
            else:
    ...
    def create_exchange_order(self, user_id: int, order_type: str, rate: float, amount: int) -> Tuple[bool, str]:
        try:
            ...
            if order_type not in ["buy", "sell"]:
                return False, "❌ Неверный тип ордера"
            ...
            return True, f"✅ Ордер создан! {order_type.upper()} {amount} 💎 по курсу {rate}"
        
        except Exception:
            return False, "❌ Ошибка"
    
    def match_exchange_orders(self, order_type: str) -> List[Tuple[ExchangeOrder, ExchangeOrder, int]]:
        matches = []
                ...
                if buy.rate >= sell.rate:
                    ...
                    if amount > 0:
                        ...
                        if buyer and seller:
                            ...
                            buyer_balance.gold -= gold_amount
                            seller_balance.premium -= amount
                            
                            asyncio.create_task(self._send_notification(buy.user_id, f"✅ Сделка исполнена! Куплено {amount} 💎 за {gold_amount} 🪙"))
                            asyncio.create_task(self._send_notification(sell.user_id, f"✅ Сделка исполнена! Продано {amount} 💎 за {gold_amount} 🪙"))
    ...
    def withdraw_exchange_balance(self, user_id: int, currency: str, amount: int) -> Tuple[bool, str]:
        try:
            ...
            balance = self.exchange_balances.get(user_id)
            if not balance:
                return False, "❌ Нет средств в обменнике"
            
            if currency == "gold":
                if balance.gold < amount:
                    return False, f"❌ Недостаточно золота в обменнике! Доступно: {balance.gold}"
                balance.gold -= amount
                player.gold_balance += amount
            elif currency == "premium":
                if balance.premium < amount:
                    return False, f"❌ Недостаточно Premium Coin в обменнике! Доступно: {balance.premium}"
                balance.premium -= amount
                player.premium_coin_balance += amount
            else:
            ...
            asyncio.create_task(self.batch_save())
            return True, f"✅ Выведено {amount} {currency.upper()}"
        ...
        try:
            ...
            if currency == "gold":
                if player.gold_balance < amount:
                    return False, f"❌ Недостаточно золота! Доступно: {player.gold_balance}"
                player.gold_balance -= amount
                balance = self.exchange_balances[user_id]
                balance.gold += amount
            elif currency == "premium":
                if player.premium_coin_balance < amount:
                    return False, f"❌ Недостаточно Premium Coin! Доступно: {player.premium_coin_balance}"
                player.premium_coin_balance -= amount
                balance = self.exchange_balances[user_id]
                balance.premium += amount
            else:
            ...
            asyncio.create_task(self.batch_save())
            return True, f"✅ Пополнено {amount} {currency.upper()}"
    ...
    def _serialize_player(self, player: Player) -> Dict:
        return {
            ...
            'referral_bonus_multiplier': float(player.referral_bonus_multiplier)
        }
    ...
    def start_mining(self, user_id: int, mineral_name: str) -> Tuple[bool, str, Dict[str, Any]]:
        try:
            ...
            session = MiningSession(
                ...
                hits_done=0,
                mineral_multiplier=player.get_mineral_multiplier() * mining_multiplier,
...
"""
    ...
    def process_auto_hit(self, user_id: int) -> Tuple[bool, Dict[str, Any]]:
        try:
            ...
            luck_chance *= getattr(session, 'luck_multiplier', 1.0)
    ...
    def process_auto_mining(self, user_id: int) -> Tuple[bool, Dict[str, Any]]:
        try:
            ...
            auto_boost_multiplier = self.get_boost_multiplier(user_id, "auto_boost")
            ...
            for mineral in auto_session.minerals:
                ...
                mineral_reward *= auto_boost_multiplier
    ...
    def complete_mining(self, user_id: int) -> Tuple[bool, Dict[str, Any]]:
        try:
            ...
            mineral_per_hit = player.get_mineral_per_hit(session.base_reward_per_hit) * session.mineral_multiplier
            ...
            cases_found = 0
            for _ in range(remaining_hits // 10 + 1):
            ...
            if cases_found > 0:
                for _ in range(cases_found):
                    ...
                    if case:
                        ...
                        dropped_items.append(case)
            
            collectible_chance = player.get_collectible_chance()
            collectibles_found = 0
            for _ in range(remaining_hits // 20 + 1):
...
"""
            ...
            return True, {
                ...
                "total_hits": session.total_hits,
                "total_time": session.total_mining_time,
    ...
    def buy_upgrade(self, user_id: int, upgrade_type: str) -> Tuple[bool, str]:
        try:
            ...
            if not player or player.is_banned:
                return False, "Шахтёр не найден или забанен"
            upgrade_map = {
                "mining_power": ("mining_power_level", 100, 1.3, "💪 Кол-во ударов"),
        ...
        async def cmd_start(message: Message):
            try:
                ...
                if len(args) > 1:
                    ...
                    if referrer_code != player.referral_code:
...
"""
        ...
        async def cmd_me(message: Message):
            ...
            text = message.text.replace("/me", "").strip()
            
            if not text:
                text = "тыкает пальцем"
            
            await message.reply(
                f"✨ {message.from_user.first_name} {text} {target.first_name}!",
                parse_mode=ParseMode.MARKDOWN
            )
        ...
        async def cmd_highfive(message: Message):
            ...
            target = message.reply_to_message.from_user
            await message.reply(f"🖐️ {message.from_user.first_name} даёт пять {target.first_name}!")
        ...
        async def cmd_slap(message: Message):
            ...
            target = message.reply_to_message.from_user
            items = ["👋 ладонью", "📖 газетой", "🎣 удочкой", "🍞 батоном", "🧀 сыром", "📀 диском"]
            item = random.choice(items)
            
            await message.reply(f"🤜 {message.from_user.first_name} ударяет {target.first_name} {item}!")
        ...
        async def cmd_hug(message: Message):
            ...
            target = message.reply_to_message.from_user
            await message.reply(f"🤗 {message.from_user.first_name} обнимает {target.first_name}!")
        ...
        async def cmd_profile_chat(message: Message):
            ...
            player = data_manager.players.get(target.id)
            if not player:
                await message.answer("❌ Игрок не найден")
                return
            
            await message.reply(
                f"👤 {player.custom_name}\n"
                f"🏆 Уровень: {player.miner_level}\n"
                f"💰 Баланс: {player.gold_balance} 🪙\n"
                f"💎 Premium Coin: {player.premium_coin_balance}\n"
                f"🔄 Перерождений: {player.reincarnation_level}\n"
                f"🤖 Авто: {'✅' if player.auto_mining_enabled else '❌'}"
            )
        ...
        async def cmd_balance_chat(message: Message):
            ...
            await message.reply(
                ...
                f"💎 Premium Coin: {player.premium_coin_balance}\n"
                f"📦 Стоимость минералов: {total_mineral:.0f} 🪙"
            )
        ...
        async def cmd_top_chat(message: Message):
            ...
            top = data_manager.get_top_players("gold", 5)
            
            text = "🏆 ТОП ПО ЗОЛОТУ 🏆\n\n"
            for i, (name, gold, level) in enumerate(top, 1):
                text += f"{i}. {name} - {gold} 🪙 (Ур.{level})\n"
